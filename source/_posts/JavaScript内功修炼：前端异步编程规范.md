...
```javascript
// 记录Promise的三种状态
const PENDING = 'pending'; // 定义常量表示Promise的状态：进行中
...
class MyPromise {
    ...
    #runMicroTask(runc) {
        ...
        if (typeof process === 'object' && typeof process.nextTick === 'function') {
            ...
            const ob = new MutationObserver(runc);
  ...
  */
    ...
    static all(promiseList) { // 定义all静态方法，用于处理多个Promise对象的并行执行
        return new MyPromise((resolve, reject) => {
            const result = []; // 存储所有Promise的结果
            let count = 0; // 计数器，用于判断所有Promise是否都已经执行完成

            const processPromise = (index, value) => { // 处理单个Promise对象的结果
                result[index] = value; // 将结果存入结果数组中
                count++; // 计数器加一
                if (count === promiseList.length) { // 如果所有Promise都已经执行完成
                    resolve(result); // 则将结果数组传递给resolve方法
                }
            };

            promiseList.forEach((promise, index) => {
                MyPromise.resolve(promise).then(
                    (value) => processPromise(index, value),
                    (reason) => reject(reason) // 修改为直接拒绝
                );
            });
        });
    }
    ...
    static allSettled(promiseList) { // 定义allSettled静态方法，用于处理多个Promise对象，无论成功或失败都会执行
        return new MyPromise((resolve) => {
            const result = []; // 存储所有Promise的结果
            let count = 0; // 计数器，用于判断所有Promise是否都已经执行完成

            const processPromise = (index, value, status) => { // 处理单个Promise对象的结果
                result[index] = { status, value }; // 将结果存入结果数组中
                count++; // 计数器加一
                if (count === promiseList.length) { // 如果所有Promise都已经执行完成
                    resolve(result); // 则将结果数组传递给resolve方法
                }
            ...
            promiseList.forEach((promise, index) => { // 遍历所有Promise对象
                MyPromise.resolve(promise).then( // 将每个元素都转换为Promise对象，并等待其执行完成
                    (value) => processPromise(index, value, FULFILLED), // 如果成功，则执行processPromise函数，标记状态为已完成
                    (reason) => processPromise(index, reason, REJECTED) // 如果失败，则执行processPromise函数，标记状态为已拒绝
                ).finally(() => {
                    count++;
    ...
    static any(promiseList) { // 定义any静态方法，用于处理多个Promise对象的竞争执行，只要有一个成功就返回成功结果
        return new MyPromise((resolve, reject) => {
            let count = 0; // 计数器，用于记录失败的Promise数量

            promiseList.forEach((promise) => { // 遍历所有Promise对象
                MyPromise.resolve(promise).then(resolve, () => { // 将每个元素都转换为Promise对象，并等待其执行完成，如果失败则执行reject方法
                    count++; // 计数器加一
                    if (count === promiseList.length) { // 如果所有Promise都已经执行完成
                        reject(new Error('All promises were rejected')); // 则将错误信息传递给reject方法
                    }
                });
            });
        });
    ...
    static race(promiseList) { // 定义race静态方法，用于处理多个Promise对象的竞争执行
        return new MyPromise((resolve, reject) => {
            if (promiseList.length === 0) {
                reject(new Error('Promise array is empty'));
            }else{
                promiseList.forEach((promise) => { // 遍历所有Promise对象
                    MyPromise.resolve(promise).then(resolve, reject); // 将每个元素都转换为Promise对象，并等待其执行完成，只要有一个执行完成就执行resolve或reject
                });
            }
    ...
    catch(onRejected) { // 定义catch方法，用于注册失败的回调函数
        return this.then(null, onRejected); // 直接调用then方法，只传入失败回调函数
    }
